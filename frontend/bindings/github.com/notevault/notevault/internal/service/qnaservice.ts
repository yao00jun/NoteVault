 ...
 */
export function Answer(apiKey: string, baseURL: string, model: string, protocol: string, embBaseURL: string, embModel: string, embAPIKey: string, rerankCfg: $models.RerankConfig, workspacePath: string, question: string): $CancellablePromise<$models.QnAResponse | null> {
    return $Call.ByName("github.com/notevault/notevault/internal/service.QnAService.Answer", apiKey, baseURL, model, protocol, embBaseURL, embModel, embAPIKey, rerankCfg, workspacePath, question);
}
 ...
 * 行为与 P1-3 结束态一致。
 */
export function HybridSearch(workspacePath: string, query: string, embBaseURL: string, embModel: string, embAPIKey: string, rerankCfg: $models.RerankConfig): $CancellablePromise<($models.SearchResult | null)[] | null> {
